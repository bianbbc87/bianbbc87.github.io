...
Kubernetes 클러스터의 **모든 상태가 etcd에 저장**되므로:
...
```bash
# snapshot-count 옵션 (기본값: 100,000)
etcd --snapshot-count=1000

# 동작:
# 1. 1000번 write마다 snapshot 생성
# 2. 메모리의 log를 파일시스템에 저장
# 3. lastIndex - 5000까지만 메모리 보관
# 4. 나머지는 truncate
```
...
# Auto compaction - Revision 모드
etcd --auto-compaction-mode=revision \
     --auto-compaction-retention=1000

# Auto compaction - Periodic 모드
...
# 초과 시: "database space exceeded" 에러
# → read-only 모드로 전환
```
...
주요 메트릭:
- **Leader 변경 횟수**: 잦은 변경은 네트워크 문제
- **디스크 동기화 시간**: 느리면 성능 저하
- **DB 크기**: 2GB 근접 시 정리 필요
- **요청 지연시간**: 성능 지표
