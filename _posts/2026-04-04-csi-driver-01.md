...
kind 클러스터 노드에 접속하여 확인하면 기본 CSI 플러그인의 존재를 직접 볼 수 있다.

```bash
# kubelet 프로세스 확인 — 기본 CSI 플러그인이 이 안에 포함되어 있다
...
# External Driver의 소켓 디렉토리 — 비어 있다
$ docker exec -it for-cri-control-plane ls /var/lib/kubelet/plugins/
(빈 결과)

# External Driver의 소켓 파일 — 없다
$ docker exec -it for-cri-control-plane find /var/lib/kubelet/plugins/ -name "csi.sock"
(빈 결과)
```

kubelet은 실행 중이고 기본 CSI 플러그인도 동작 중이지만, **External Driver가 없으므로 gRPC 소켓이 존재하지 않는다**.

클러스터 외부에서 확인해보면 더 명확하다.
...
# CSINode — 기본 플러그인이 자동 생성했지만 drivers는 비어 있음
...
> kubelet은 준비 완료 상태이지만, **상대방(연주자)이 없는 상태**다.
