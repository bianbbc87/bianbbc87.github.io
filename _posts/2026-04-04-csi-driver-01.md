...
`migrated-plugins` 어노테이션은 **"이 in-tree 드라이버들은 CSI로 우회시켜라"라는 라우팅 테이블**이다.

예를 들어 누군가 기존 방식(`awsElasticBlockStore`)으로 PV를 만들면:

```
PV spec에 awsElasticBlockStore 사용
    ↓
kubelet: "kubernetes.io/aws-ebs가 migrated-plugins에 있네"
    ↓
in-tree 코드로 처리하지 않고, ebs.csi.aws.com External Driver로 우회
```

| migrated-plugins에 있으면 | migrated-plugins에 없으면 |
|---|---|
| in-tree 코드 무시 → **CSI Driver로 우회** | in-tree 코드가 직접 처리 |

지금 이 kind 클러스터는 어노테이션은 있는데 External Driver가 없는 상태다. 즉, CSI로 우회하려는데 **받아줄 Driver가 없어서 실패하는 상태**다.
